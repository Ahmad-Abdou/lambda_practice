...
public class DataStorageImpl implements DataStorage {
    ...
    public List<String> findManyAndMapEachToString(Predicate<Person> filter, Function<Person, String> personToString) {
        List<Person> personFilter = findMany(filter);
        List<String> stringList = new ArrayList<>();

        for (Person person : personFilter) {
            String str = personToString.apply(person);
            stringList.add(str);
    ...
    public List<Person> findAndSort(Comparator<Person> comparator) {
        //List<Person> copy = personList;
        List<Person> copy = new ArrayList<>(personList);//this is the same as personList
